...
function Dashboard (){
  ...
  /*  const handlePollBlockVisibility = (index) => {
      setIsPollBlockVisible(true);
      
      // Ensure there's at least one option input when opening the Poll block
      if (inputs.length === 0) {
  ...
  const handleInputChange = (index, event, type) => {
    ...
    const currentQuestionIsValid = () => {
      return currentQuestion.trim() !== '';  // Check if the question is non-empty
    };

    const updateQuestion = (index, newQuestionData) => {
      setCircles(prevCircles =>
          prevCircles.map((circle, i) =>
              i === index ? { ...circle, questionData: newQuestionData } : circle
          )
      );
  };
  ...
  {selectedDot === 2 && (
   ...
   {index >= 2 && (
    ...
    />
        ...
        <button style={{width: '279px',height:'46.14px',borderRadius:'10px',boxShadow:'0px 0px 15px 0px #00000040',background:'#60B84B',
          color:'#FFFFFF', border: 'none', outline: 'none',fontFamily: 'Poppins, sans-serif', fontSize: '25px', fontWeight: '500',
           lineHeight: '37.5px', textAlign: 'center'}} onClick={async () => {
            handleSaveQuestion(); // Save the last question
            await handleQuizName(); 
          }}>Create Quiz</button>
      </div>
      </div>
    </div>
